...
namespace CardCloud.Tests
{
    public abstract class CardCloudTestBase : AbpIntegratedTestBase<CardCloudTestModule>
    {
        ...
        protected void UsingDbContext(Action<CardCloudDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<CardCloudDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<CardCloudDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<CardCloudDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<CardCloudDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<CardCloudDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<CardCloudDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<CardCloudDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<CardCloudDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<CardCloudDbContext, Task<T>> func)
        {
            T result;
